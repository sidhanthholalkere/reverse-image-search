...
def choose_bad_img(good_img_caption, images, cap2vec, imgid2capid):
    """
    ...
    dist = np.array([np.matmul(encoding / np.linalg.norm(encoding), good_img_caption) for encoding in bad_captions])
...
def all_triplets(path):
    ...
    for i, indiv_caption_id in enumerate(caption_id):
        ...
        caption = caption / np.linalg.norm(caption)
        ...
        for i in range(10):
            while True:
                ...
                if img_id not in images:
                    break
            img_key = choose_bad_img(caption, images, caption_id_to_caption, img_id_to_caption_id)
